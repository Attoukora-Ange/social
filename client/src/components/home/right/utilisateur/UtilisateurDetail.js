...
export const UtilisateurDetail = ({ membre }) => {
    ...
    <List sx={{ width: "100%", p: 0 }}>
      {user?._id !== membre._id && (
        <ListItem sx={{ width: 250 }} disablePadding>
          ...
          {user?.following.includes(membre._id) ? (
            // Bouton de suppression si l'utilisateur suit déjà le membre
            <Tooltip onClick={handleDelete} title="Supprimer">
              <RemoveCircleIcon color="error" />
            </Tooltip>
          ) : (
            // Bouton de suivi si l'utilisateur ne suit pas encore le membre
