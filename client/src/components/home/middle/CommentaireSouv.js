...
export const CommentaireSouv = ({ handleExpandClick, postId, comment }) => {
  ...
  const handleDeleteCommentaire = async (id) => {
    try {
      ...
      const AXIOS_TO_GET_SOUVENIR = async () => {
        const token = JSON.parse(localStorage.getItem("token"));
        const OPTIONS = {
          headers: {
      ...
      // Appeler la fonction pour rafraîchir la liste des souvenirs
      await AXIOS_TO_GET_SOUVENIR().catch(() => {
        dispatch({ type: ERROR });
      });
    } catch (error) {
      dispatch({ type: ERROR });
